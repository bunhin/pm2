...
UX.dispAsTable = function(list) {
  var table = new Table({
    head:      ['User', 'Process', 'URL',  'Port', 'Uptime', 'RAM',   'CPU',  'Schedule', 'Status', 'PID'],
  ...
  list.forEach(function(l) {
    ...
    var port = l.pm2_env.port;
    var key = l.pm2_env.name || p.basename(l.pm2_env.pm_exec_path.script);
    var keyTokens = env.XT_NAME.split('-'),
      processName = (keyTokens[0] + '-' + keyTokens[1] + '-' + keyTokens[2]).white.bold;

    var user = env.XT_NAME.white;
    ...
    obj[user] = [
      ...
      (l.monit && l.monit.cpu || 0).toFixed(0) + ' %',
