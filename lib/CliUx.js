...
UX.dispAsTable = function(list) {
  var table = new Table({
    head:      ['User', 'Process', 'URL',  'Port', 'RAM',   'CPU',  'Schedule', 'Uptime', 'Status'],
  ...
  list.forEach(function(l) {
    ...
    var status = (function () {
      if (l.pm2_env.status === 'online') {
        return 'ONLINE'.bold.green;
      }
      if (env.XT_SCHEDULE !== 'Always On') {
        return 'SLEEPING'.bold.white;
      }
      else {
    ...
    obj[user] = [
      (env.XT_PROCESS || '--').white.bold,
      (env.XT_URL || '--').cyan,
      (port ? port : env.PORT) || '--',
      (l.monit && l.monit.memory > 0) ? UX.bytesToSize(l.monit.memory, 0) : '--',
      (l.monit && l.monit.cpu || 0).toFixed(0) + ' %',
      env.XT_SCHEDULE || '--',
      (l.pm2_env.pm_uptime && l.pm2_env.status === 'online') ? timeSince(l.pm2_env.pm_uptime) : '--',
      status
    ];
