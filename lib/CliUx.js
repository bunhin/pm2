...
UX.dispAsTable = function(list) {
  var table = new Table({
    head:       ['Name', 'Process', 'URL',  'PG Port', 'Uptime', 'RAM',   'CPU',  'Schedule',  'Mode', 'Status', 'PID'],
    colAligns : ['left', 'left',    'left', 'left',    'left',   'left',  'left', 'left',      'left', 'left',   'left'],
  ...
  list.forEach(function(l) {
    ...
    var env = l.pm2_env.env;
    var mode = l.pm2_env.exec_mode.split('_mode')[0];
    var status = l.pm2_env.status;
    var port = l.pm2_env.port;
    var key = l.pm2_env.name || p.basename(l.pm2_env.pm_exec_path.script);

    var name = (env.XT_NAME || key).white.bold;

    obj[name] = [
      key,
      env.XT_URL || '--',
      (port ? port : env.XT_PG_PORT) || '--',
      (l.pm2_env.pm_uptime && status == 'online') ? timeSince(l.pm2_env.pm_uptime) : '--',
      (l.monit && l.monit.memory > 0) ? UX.bytesToSize(l.monit.memory, 0) : '--',
      (l.monit.cpu || 0).toFixed(0) + ' %',
      env.XT_SCHEDULE || 'Always On',
      mode || '--',
      status === 'online' ? 'ONLINE'.bold.green : 'OFFLINE'.bold.red,
      l.pid || '--'
    ];
